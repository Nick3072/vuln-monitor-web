...
import { canWriteGroup, requireAdmin, defaultGroupForUser } from '../middleware/permissions'
...
const BULK_CSV_TEMPLATE = `vendor,model,hostname,os_version,hw_version,db_engine,db_version,openssl_version,web_engine,web_version,was_engine,was_version,group_company,owner,notes
Fortinet,FortiGate-100F,fw-hq-01,7.4.1,,,,1.1.1k,,,,,본사,보안팀,HA Primary
Microsoft,Windows Server,srv-app-prod-01,2022,,MSSQL,2019,1.1.1k,IIS,10.0,,,본사,인프라팀,Std Edition
Canonical,Ubuntu Server,web-front-01,22.04,,MySQL,8.0.36,3.0.5,Apache,2.4.58,Tomcat,9.0.85,자회사A,웹운영팀,Production
...
app.get('/', async (c) => {
  ...
  const [stats, recent, groups, allMatches, categories, assetTotalRow] = await Promise.all([
    db
      .prepare(
        `SELECT
...
app.get('/solutions', async (c) => {
  ...
  const [solutionsRes, groupsRes, assetGroups, unlinkedCount, assetOptions] = await Promise.all([
    ...
    db
      ...
      .all<GroupSummary>(),
    // v3.1: 그룹 뷰용 자산+컴포넌트 집계
    getAssetsWithComponents(db, { group, category }),
...
app.post('/solutions', async (c) => {
  ...
  if (!parsed.group_company) {
    parsed.group_company = defaultGroupForUser(c)
  }
...
app.post('/solutions/:id', async (c) => {
  ...
  if (!parsed.group_company) {
    parsed.group_company = defaultGroupForUser(c) ?? existing.group_company
  }
...
app.post('/solutions/assets/backfill', async (c) => {
  ...
  return redirectFlash(
    c,
    'success',
    `${r.assetsCreated}개 솔루션 생성 · ${r.componentsLinked}개 구성요소 연결`,
  )
})
